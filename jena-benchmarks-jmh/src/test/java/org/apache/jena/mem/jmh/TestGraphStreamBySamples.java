...
public class TestGraphStreamBySamples extends AbstractJmhTestGraphBase {
    ...
    @Benchmark
    public int graphStreamBySamples_Subject_ANY_ANY() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(sample.getSubject(), Node.ANY, Node.ANY));
        }
        assertTrue(total > 0);
        return total;
    }
    ...
    @Benchmark
    public int graphStreamBySamples_ANY_Predicate_ANY() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(Node.ANY, sample.getPredicate(), Node.ANY));
        }
        assertTrue(total > 0);
        return total;
    }
    ...
    @Benchmark
    public int graphStreamBySamples_ANY_ANY_Object() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(Node.ANY, Node.ANY, sample.getObject()));
        }
        assertTrue(total > 0);
        return total;
    }
    ...
    @Benchmark
    public int graphStreamBySamples_Subject_Predicate_ANY() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(sample.getSubject(), sample.getPredicate(), Node.ANY));
        }
        assertTrue(total > 0);
        return total;
    }
    ...
    @Benchmark
    public int graphStreamBySamples_Subject_ANY_Object() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(sample.getSubject(), Node.ANY, sample.getObject()));
        }
        assertTrue(total > 0);
        return total;
    }
    ...
    @Benchmark
    public int graphStreamBySamples_ANY_Predicate_Object() {
        var total = 0;
        for (Triple sample : samples) {
            total += count(sut.stream(Node.ANY, sample.getPredicate(), sample.getObject()));
        }
        assertTrue(total > 0);
        return total;
    }
