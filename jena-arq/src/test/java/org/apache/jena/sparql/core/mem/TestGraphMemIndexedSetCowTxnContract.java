...
/**
 * Graph-level contract checks for {@link GraphMemIndexedSetCowTxn} that
 * complement the unit tests in {@link GraphMemIndexedSetCowTxnTest} and
 * {@link GraphMemIndexedSetCowTxnStrategyTest} by walking the
 * Graph-shaped surface (add / delete / find / contains / size /
 * iterator/stream) within explicit transactions, across all five
 * indexing strategies.
 * <p>
 * This is the test that pins down "the COW graph is a well-behaved Graph
 * implementation under transactions" — the broader transaction semantics
 * (promote, lock contention, snapshot stability) are covered elsewhere.
 * <p>
 * Bundled into the {@link TS_DatasetTxnMem} suite so it is exercised
 * alongside the existing in-memory dataset tests.
 */
public class TestGraphMemIndexedSetCowTxnContract {
    ...
    private static GraphMemIndexedSetCowTxn populated(IndexingStrategy s, int n) {
    ...
    public void containsAndFindAgreeAcrossAllStrategiesForFullyConcrete() {
        // SUB_PRE_OBJ goes straight through the triple set, bypassing
        // the strategy. Still — every strategy must answer correctly.
        for (IndexingStrategy s : IndexingStrategy.values()) {
            GraphMemIndexedSetCowTxn g = populated(s, 5);
            g.begin(TxnType.READ);
    ...
    @Test
    public void streamPartialPatternsAgreeWithEagerForAllStrategies() {
        // For every non-MANUAL strategy: streaming a partial pattern
        // must yield the same set as eager.
        IndexingStrategy[] strategies = {
        ...
        {
            GraphMemIndexedSetCowTxn eager = populated(IndexingStrategy.EAGER, 8);
        ...
        for (IndexingStrategy s : strategies) {
            GraphMemIndexedSetCowTxn g = populated(s, 8);
            g.begin(TxnType.READ);
    ...
    public void abortDiscardsBothAddsAndDeletes() {
        GraphMemIndexedSetCowTxn g = populated(IndexingStrategy.EAGER, 3);
    ...
    public void readSnapshotIsStableWhileWriterRunsFreshTxns() throws Exception {
        // Different shape than GraphMemIndexedSetCowTxnTest's reader-isolation
        // test: this drives many small commit cycles and verifies the
        // reader's view doesn't twitch — exercises republish under churn.
        GraphMemIndexedSetCowTxn g = populated(IndexingStrategy.EAGER, 10);

        g.begin(TxnType.READ);
        try {
            int initialSize = g.size();
            // Run many writers in another thread.
            ...
            writer.start();
            // Reader keeps polling its snapshot — must never observe a
            // size change.
            for (int k = 0; k < 50; k++) {
        ...
        // Fresh read sees the writer's commits.
        g.begin(TxnType.READ);
    ...
    public void findWithAnyAnyAnyEnumeratesAllTriples() {
        GraphMemIndexedSetCowTxn g = populated(IndexingStrategy.EAGER, 5);
        g.begin(TxnType.READ);
    ...
    public void streamOutsideTransactionSeesPublished() {
        GraphMemIndexedSetCowTxn g = populated(IndexingStrategy.EAGER, 3);
        // Stream without an active transaction — should reflect the
        // currently published view (which is fully indexed).
