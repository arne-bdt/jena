...
/**
 * The Bifurcan-backed counterpart of {@link TestHexTable}.
 */
public class TestBifurcanHexTable extends AbstractTestQuadTable {

	@Test
	public void testListGraphNodes() {
		final int nodesToTry = 50;
		final BifurcanHexTable index = new BifurcanHexTable();
	...
	public void checkConcreteQueries() {
		queryPatterns().filter(p -> !allWildcardQuery.equals(p)).map(TestBifurcanHexTable::exampleFrom).forEach(testQuery -> {
			final BifurcanHexTable index = new BifurcanHexTable();
			index.begin(null);
			// add our sample quad
			index.add(testTuple());
	...
	private final BifurcanHexTable testTable = new BifurcanHexTable();
