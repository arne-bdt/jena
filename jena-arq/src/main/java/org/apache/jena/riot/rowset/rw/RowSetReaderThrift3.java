...
public class RowSetReaderThrift3 implements RowSetReader {

    public static RowSetReaderFactory factory = lang->{
        if (!Objects.equals(lang, ResultSetLang.RS_Thrift3 ) )
            throw new ResultSetException("RowSetReaderThrift3 for Thrift3 asked for a "+lang);
        return new RowSetReaderThrift3();
    };

    private RowSetReaderThrift3() {}

    @Override
    public RowSet read(InputStream in, Context context) {
        return Thrift3RDF.readRowSet(in);
    }
