...
public class RowSetReaderThrift2 implements RowSetReader {

    public static RowSetReaderFactory factory = lang->{
        if (!Objects.equals(lang, ResultSetLang.RS_Thrift2 ) )
            throw new ResultSetException("RowSetReaderThrift2 for Thrift2 asked for a "+lang);
        return new RowSetReaderThrift2();
    };

    private RowSetReaderThrift2() {}

    @Override
    public RowSet read(InputStream in, Context context) {
        return Thrift2RDF.readRowSet(in);
    }
