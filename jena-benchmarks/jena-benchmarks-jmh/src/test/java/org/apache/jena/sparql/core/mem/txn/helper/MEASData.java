...
/**
 * This class provides methods to generate random MEAS data and to add this data to a Jena graph.
 */
public class MEASData {
    ...
    public static List<AnalogValue> generateRandomAnalogValues(int numberOfAnalogValues) {
        ...
        for (int i = 0; i < numberOfAnalogValues; i++) {
            list.add(new AnalogValue("urn:uuid:" + UUID.randomUUID(), random.nextFloat(), Clock.systemUTC().instant(), random.nextInt()));
        }
        return list;
    }
    ...
    public static List<DiscreteValue> generateRandomDiscreteValues(int numberOfDiscreteValues) {
        ...
        for (int i = 0; i < numberOfDiscreteValues; i++) {
            list.add(new DiscreteValue("urn:uuid:" + UUID.randomUUID(), random.nextInt(), Clock.systemUTC().instant(), random.nextInt()));
        }
        return list;
    }
    ...
    /**
     * Generates only the triples for value, status and timestamp.
     * @param analogValues
     * @return
     */
    public static List<Triple> analogValuesToTriplesForUpdate(final List<AnalogValue> analogValues) {
        ...
        for (final var analogValue : analogValues) {
            final var subject = NodeFactory.createURI("urn:uuid:" + analogValue.uuid());
            triples.add(Triple.create(subject, MeasurementValueTimeStamp.asNode(), NodeFactory.createLiteralDT(DateTimeFormatter.ISO_INSTANT.format(analogValue.timeStamp()), XSDDatatype.XSDdateTimeStamp)));
            triples.add(Triple.create(subject, MeasurementValueStatus.asNode(), NodeFactory.createLiteralByValue(analogValue.status(), XSDDatatype.XSDinteger)));
            triples.add(Triple.create(subject, AnalogValueValue.asNode(), NodeFactory.createLiteralByValue(analogValue.value(), XSDDatatype.XSDfloat)));
        }
        return triples;
    }
     ...
     * @return
     */
    public static List<Triple> disccreteValuesToTriplesForUpdate(final List<DiscreteValue> discreteValues) {
        final var triples = new ArrayList<Triple>(discreteValues.size() * 3);
        for (final var discreteValue : discreteValues) {
            final var subject = NodeFactory.createURI("urn:uuid:" + discreteValue.uuid());
            triples.add(Triple.create(subject, MeasurementValueTimeStamp.asNode(), NodeFactory.createLiteralDT(DateTimeFormatter.ISO_INSTANT.format(discreteValue.timeStamp()), XSDDatatype.XSDdateTimeStamp)));
            triples.add(Triple.create(subject, MeasurementValueStatus.asNode(), NodeFactory.createLiteralByValue(discreteValue.status(), XSDDatatype.XSDinteger)));
            triples.add(Triple.create(subject, AnalogValueValue.asNode(), NodeFactory.createLiteralByValue(discreteValue.value(), XSDDatatype.XSDinteger)));
        }
        return triples;
    }
    ...
    public static void addAnalogValuesToGraph(final Graph graph, final List<AnalogValue> analogValues) {
        ...
        for (final var analogValue : analogValues) {
            model.createResource("urn:uuid:" + analogValue.uuid())
    ...
    public static void addDiscreteValuesToGraph(final Graph graph, final List<DiscreteValue> discreteValues) {
        ...
        for (final var discreteValue : discreteValues) {
            model.createResource("urn:uuid:" + discreteValue.uuid())
