...
public class MEASData {
    ...
    public static List<Triple> analogValuesToTriplesForUpdate(final List<AnalogValue> analogValues) {
        ...
        for (final var analogValue : analogValues) {
            final var subject = NodeFactory.createURI("xx://" + analogValue.uuid());
    ...
    public static List<Triple> disccreteValuesToTriplesForUpdate(final List<DiscreteValue> discreteValues) {
        ...
        for (final var discreteValue : discreteValues) {
            final var subject = NodeFactory.createURI("xx://" + discreteValue.uuid());
    ...
    public static void addAnalogValuesToGraph(final Graph graph, final List<AnalogValue> analogValues) {
        ...
        for (final var analogValue : analogValues) {
            model.createResource("xx://" + analogValue.uuid())
                    .addProperty(RDF.type, model.createResource(MEAS_NS + analogTypeProvider.next()))
                    .addProperty(AnalogValueAnalog, model.createResource("xx://_" + UUID.randomUUID()))
    ...
    public static void addDiscreteValuesToGraph(final Graph graph, final List<DiscreteValue> discreteValues) {
        ...
        for (final var discreteValue : discreteValues) {
            model.createResource("xx://" + discreteValue.uuid())
                    .addProperty(RDF.type, model.createResource(MEAS_NS + discreteTypeProvider.next()))
                    .addProperty(DiscreteValueDiscrete, model.createResource("xx://_" + UUID.randomUUID()))
