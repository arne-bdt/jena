 ...
 */
public class TestCacheSimpleFastConcurrent {

    /**
     * Simple test to ensure that {@link CacheSimpleFast} evidences
    ...
    public void testFixedSize() {
        ...
        final Cache<Integer, Object> testCache = new CacheSimpleFastConcurrent<>(maxSize);
    ...
    public void testReplace() {
        ...
        final Cache<Integer, Object> testCache = new CacheSimpleFastConcurrent<>(5);
    ...
    public void testSameHash() {
        ...
        Cache<CompoundKey, Integer> cache = new CacheSimpleFastConcurrent<>(10);
        cache.put(key1, 1);
    ...
    public void testPutSameHashOverridesValue() {
        ...
        Cache<CompoundKey, String> cache = new CacheSimpleFastConcurrent<>(10);
    ...
    public void removeKeyByPutingNullValue() {
        Cache<String, String> cache = new CacheSimpleFastConcurrent<>(10);
        assertEquals(0, cache.size());
    ...
    @Test
    public void testWeakReferencesInCache() {
        var cache = new CacheSimpleFastConcurrent<String, CompoundKey>(16);
        Assert.assertFalse(cache.keys().hasNext());
        Assert.assertFalse(cache.values().hasNext());
        {
            cache.get(new String("http://example.org/4711"), s -> new CompoundKey(s.hashCode(), 4711));
            Assert.assertTrue(cache.containsKey("http://example.org/4711"));

            Assert.assertTrue(cache.keys().hasNext());
            Assert.assertTrue(cache.values().hasNext());
        }
        System.gc();
        System.runFinalization();
        System.gc();
        System.runFinalization();
        Assert.assertFalse(cache.containsKey("http://example.org/4711"));
        Assert.assertFalse(cache.keys().hasNext());
        Assert.assertFalse(cache.values().hasNext());
    }
}
