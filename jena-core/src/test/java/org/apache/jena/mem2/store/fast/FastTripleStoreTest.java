...
public class FastTripleStoreTest extends AbstractTripleStoreTest {
    ...
    protected TripleStore createTripleStore() {
        return new org.apache.jena.mem2.store.fast.FastTripleStore();
    }
    ...
    public void testFindStreamAndContains_POOptimizationWithFewerPredicateMatches() {
        for (int i = 0; i < org.apache.jena.mem2.store.fast.FastTripleStore.THRESHOLD_FOR_SECONDARY_LOOKUP + 1; i++) {
            sut.add(triple("s" + i + " P" + i + " o"));
        }
        assertEquals(org.apache.jena.mem2.store.fast.FastTripleStore.THRESHOLD_FOR_SECONDARY_LOOKUP + 1, sut.countTriples());
    ...
    public void testFindStreamAndContains_POOptimizationWithMorePredicateMatches() {
        ...
        assertEquals(org.apache.jena.mem2.store.fast.FastTripleStore.THRESHOLD_FOR_SECONDARY_LOOKUP + 2, sut.countTriples());

        assertEquals(org.apache.jena.mem2.store.fast.FastTripleStore.THRESHOLD_FOR_SECONDARY_LOOKUP + 1, sut.find(triple("?? P o")).toList().size());
