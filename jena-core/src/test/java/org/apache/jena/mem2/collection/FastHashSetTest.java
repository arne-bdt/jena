...
/**
 * This test shall test only the parts of the {@link FastHashSet} which are not tested by the {@link JenaSetTest}.
 */
public class FastHashSetTest {
    ...
    public void testGetKeyAt() {
        assertEquals(sut.addAndGetIndex("a"), 0);
        assertEquals(sut.addAndGetIndex("b"), 1);
        assertEquals(sut.addAndGetIndex("c"), 2);
    ...
    private static class FastObjectHashSet extends FastHashSet<Object> {
        ...
        public FastObjectHashSet(int initialSize) {
            super(initialSize);
        }
