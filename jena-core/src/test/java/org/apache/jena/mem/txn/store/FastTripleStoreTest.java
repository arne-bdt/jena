...
public class FastTripleStoreTest extends AbstractTripleStoreTest {
    ...
    protected TripleStore createTripleStore() {
        return new FastTripleStorePersistable();
    }

    @Test
    public void testAddMoreTriplesThanFitInArrayBunchSameSubject() {
        for (int i = 0; i < FastTripleStorePersistable.MAX_ARRAY_BUNCH_SIZE_SUBJECT + 1; i++) {
            sut.add(triple("s P" + i + " o" + i));
        }
        assertEquals(FastTripleStorePersistable.MAX_ARRAY_BUNCH_SIZE_SUBJECT + 1, sut.countTriples());
    }

    @Test
    public void testAddMoreTriplesThanFitInArrayBunchSamePredicate() {
        for (int i = 0; i < FastTripleStorePersistable.MAX_ARRAY_BUNCH_SIZE_PREDICATE_OBJECT + 1; i++) {
            sut.add(triple("s" + i + " P o" + i));
        }
        assertEquals(FastTripleStorePersistable.MAX_ARRAY_BUNCH_SIZE_PREDICATE_OBJECT + 1, sut.countTriples());
    }
    ...
    public void testFindStreamAndContains_POOptimizationWithFewerPredicateMatches() {
        for (int i = 0; i < FastTripleStorePersistable.THRESHOLD_FOR_SECONDARY_LOOKUP + 1; i++) {
            sut.add(triple("s" + i + " P" + i + " o"));
        }
        assertEquals(FastTripleStorePersistable.THRESHOLD_FOR_SECONDARY_LOOKUP + 1, sut.countTriples());
    ...
    public void testFindStreamAndContains_POOptimizationWithMorePredicateMatches() {
        ...
        assertEquals(FastTripleStorePersistable.THRESHOLD_FOR_SECONDARY_LOOKUP + 2, sut.countTriples());

        assertEquals(FastTripleStorePersistable.THRESHOLD_FOR_SECONDARY_LOOKUP + 1, sut.find(triple("?? P o")).toList().size());
        assertEquals(FastTripleStorePersistable.THRESHOLD_FOR_SECONDARY_LOOKUP + 1, sut.stream(triple("?? P o")).count());
