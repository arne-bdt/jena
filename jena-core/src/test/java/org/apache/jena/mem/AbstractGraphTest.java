...
/**
 * Contract tests exercised through the plain {@link Graph} interface
 * (add / delete / find / contains / size / stream / clear). Any {@link Graph}
 * implementation can be tested by supplying a {@link #createGraph()} factory.
 * <p>
 * The type parameter {@code G} lets a subclass narrow {@code sut} to a more
 * specific graph type (e.g. {@link AbstractGraphMemTest} uses
 * {@code AbstractGraphTest<GraphMem>}); the tests here only ever use it as a
 * {@link Graph}. Tests that depend on {@link org.apache.jena.mem.GraphMem}-specific
 * behaviour (notably {@code copy()} / {@link org.apache.jena.atlas.lib.Copyable})
 * live in the {@link AbstractGraphMemTest} subclass.
 */
public abstract class AbstractGraphTest<G extends Graph> {
