...
public class SparseArraySubSpliterator<E> implements Spliterator<E> {
     ...
     * @param deleted                        the deleted flags array
     * @param fromIndex                      the index of the first element, inclusive
     * @param toIndex                        the index of the last element, exclusive
     * @param checkForConcurrentModification runnable to check for concurrent modifications
     */
    public SparseArraySubSpliterator(final E[] entries, final boolean[] deleted, final int fromIndex, final int toIndex, final Runnable checkForConcurrentModification) {
     ...
     * @param checkForConcurrentModification runnable to check for concurrent modifications
     */
    public SparseArraySubSpliterator(final E[] entries, final boolean[] deleted, final Runnable checkForConcurrentModification) {
        this(entries, deleted, 0, entries.length, checkForConcurrentModification);
    }
    ...
    public Spliterator<E> trySplit() {
        ...
        return new SparseArraySubSpliterator<>(entries, this.deleted, this.pos, toIndexOfSubIterator, checkForConcurrentModification);
    }
