 ...
 * @param <E>
 */
public class SparseArraySubSpliteratorMapping<E, T> implements Spliterator<E> {
     ...
     */
    public SparseArraySubSpliteratorMapping(final T[] entries, final int fromIndex, final int toIndex, final Function<T, E> mapper, final float fillRatio, final Runnable checkForConcurrentModification) {
     ...
     */
    public SparseArraySubSpliteratorMapping(final T[] entries, final int estimatedElementsCount, final Function<T, E> mapper, final Runnable checkForConcurrentModification) {
       this(entries, 0, entries.length, mapper, ((float)estimatedElementsCount / (float)entries.length), checkForConcurrentModification);
    }
    ...
    public boolean tryAdvance(Consumer<? super E> action) {
        ...
        while(fromIndex <= --pos) {
            if(null != entries[pos]) {
                action.accept(mapper.apply(entries[pos]));
                return true;
            }
        }
        return false;
    ...
    public Spliterator<E> trySplit() {
        ...
        return new SparseArraySubSpliteratorMapping<>(entries, this.pos, toIndexOfSubIterator, this.mapper, fillRatio, checkForConcurrentModification);
    }
