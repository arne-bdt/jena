...
public class HashSetOfTripleSets2 {
    ...
    protected TripleSetWithIndexingValue2[] entries;
    protected int[] hashCodes;

    public HashSetOfTripleSets2() {
        this.entries = new TripleSetWithIndexingValue2[MINIMUM_SIZE];
        this.hashCodes = new int[MINIMUM_SIZE];

    }

    public HashSetOfTripleSets2(int initialCapacity) {
        this.entries = new TripleSetWithIndexingValue2[Integer.highestOneBit(((int)(initialCapacity/loadFactor)+1)) << 1];
        this.hashCodes = new int[entries.length];
    }

    public HashSetOfTripleSets2(Set<TripleSetWithIndexingValue2> set) {
        this(set.size(), set);
    }

    public HashSetOfTripleSets2(int initialCapacity, Set<TripleSetWithIndexingValue2> set) {
        this.entries = new TripleSetWithIndexingValue2[Integer.highestOneBit(((int)(Math.max(set.size(), initialCapacity)/loadFactor)+1)) << 1];
        this.hashCodes = new int[entries.length];
        int index, hashCode;
        Object indexingValue;
        for (TripleSetWithIndexingValue2 e : set) {
    ...
    private boolean grow() {
        ...
        this.entries = new TripleSetWithIndexingValue2[newSize];
    ...
    public boolean contains(Object o) {
        var e = (TripleSetWithIndexingValue2)o;
    ...
    public Iterator<TripleSetWithIndexingValue2> iterator() {
        return new ArrayWithNullsIterator(entries, size);
    }

    public TripleSetWithIndexingValue2 getIfPresent(final Object indexingValue) {
    ...
    public TripleSetWithIndexingValue2 compute(final Object key, final int hashCodeOfKey, Function<TripleSetWithIndexingValue2, TripleSetWithIndexingValue2> remappingFunction) {
    ...
    public TripleSetWithIndexingValue2 compute(final Object key, Function<TripleSetWithIndexingValue2, TripleSetWithIndexingValue2> remappingFunction) {
     ...
     */
    public void clear() {
        entries = new TripleSetWithIndexingValue2[MINIMUM_SIZE];
        hashCodes = new int[MINIMUM_SIZE];
        size = 0;
    }


    public Stream<TripleSetWithIndexingValue2> stream() {
        return StreamSupport.stream(new ArrayWithNullsSpliteratorSized(entries, size), false);
    }


    public Stream<TripleSetWithIndexingValue2> parallelStream() {
        return StreamSupport.stream(new ArrayWithNullsSpliteratorSized(entries, size), true);
    }

    private static class ArrayWithNullsSpliteratorSized implements Spliterator<TripleSetWithIndexingValue2> {

        private final TripleSetWithIndexingValue2[] entries;
        private final int maxPos;
        private int pos = -1;
        private int maxRemaining;
        private boolean hasBeenSplit = false;

        public ArrayWithNullsSpliteratorSized(final TripleSetWithIndexingValue2[] entries, final int size) {
         ...
         */
        @Override
        public boolean tryAdvance(Consumer<? super TripleSetWithIndexingValue2> action) {
            if(0 < maxRemaining) {
                while (pos < maxPos) {
                    if (null != entries[++pos]) {
                        maxRemaining--;
         ...
         */
        @Override
        public void forEachRemaining(Consumer<? super TripleSetWithIndexingValue2> action) {
         ...
         */
        @Override
        public Spliterator<TripleSetWithIndexingValue2> trySplit() {
    ...
    private static class ArrayWithNullsSubSpliteratorUnSized implements Spliterator<TripleSetWithIndexingValue2> {
        ...
        public ArrayWithNullsSubSpliteratorUnSized(final TripleSetWithIndexingValue2[] entries, final int fromIndex, final int toIndex, final int maxSize) {
            this.entries = entries;
    ...
    private static class ArrayWithNullsIterator implements Iterator<TripleSetWithIndexingValue2> {

        private final TripleSetWithIndexingValue2[] entries;
        private int remaining;
        private int pos = -1;

        private ArrayWithNullsIterator(final TripleSetWithIndexingValue2[] entries, final int size) {
            this.entries = entries;
            this.remaining = size;
        }
         ...
         */
        @Override
        public TripleSetWithIndexingValue2 next() {
            if(0 < remaining--) {
