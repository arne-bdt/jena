...
public abstract class HashedSetBase2<Key> {
    /**
    ...
    public HashedSetBase2(int initialCapacity) {
    ...
    protected final int initialIndexFor(int hashOfKey) {
        return (hashOfKey & 0x7fffffff) % keys.length;
    }
    ...
    protected int calcGrownCapacityAndSetThreshold() {
        final var capacity = HashedSetBase2.nextSize(keys.length * 2);
