...
package org.apache.jena.mem2.store.roaringLowMem;
...
public class RoaringLowMemTripleStore implements TripleStore {

    private static final RoaringBitmap EMPTY_BITMAP = new RoaringBitmap();
    SubjectToBitmapsMap subjectBitmaps = new SubjectToBitmapsMap();
    ...
    private class SubjectToBitmapsMap extends NodesToBitmapsMap {

        @Override
        protected Node getKey(RoaringBitmap integers) {
            return triples.getKeyAt(integers.first()).getSubject();
        }
    }

    private class PredicateToBitmapsMap extends NodesToBitmapsMap {

        @Override
        protected Node getKey(RoaringBitmap integers) {
            return triples.getKeyAt(integers.first()).getPredicate();
        }
    }

    private class ObjectToBitmapsMap extends NodesToBitmapsMap {

        @Override
        protected Node getKey(RoaringBitmap integers) {
            return triples.getKeyAt(integers.first()).getObject();
        }
    }
