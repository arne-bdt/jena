 ...
 * has been fixed.
 */
public class GraphMem2UsingHashes extends GraphMemBase implements GraphWithPerform {

    private TriplesBySubject triplesBySubject = new TriplesBySubject();
    ...
    public class TriplesBySubject extends FastHashMapBase<TripleSet>  {
        @Override
        ...
        public boolean addWithHashCodes(TripleWithHashCodes tripleWithHashCodes) {
            return this.getOrCreate(tripleWithHashCodes.hashes[1], () -> new TripleSet())
        ...
        public boolean removeWithHashCodes(TripleWithHashCodes tripleWithHashCodes) {
            ...
            this.removeIf(tripleWithHashCodes.hashes[1],
                    (s) -> {
                        if (removed[0] = s.remove(tripleWithHashCodes.triple, tripleWithHashCodes.hashes[0])) {
                            return s.isEmpty();
                        }
                        return false;
                    });
    ...
    public class TriplesByPredicate extends FastHashMapBase<TripleSet>  {
        @Override
        protected TripleSet[] createEntryArray(int length) {
            return new TripleSet[length];
        }
        ...
        public void addWithHashCodesUnsafe(TripleWithHashCodes tripleWithHashCodes) {
            this.getOrCreate(tripleWithHashCodes.hashes[2], () -> new TripleSet())
        ...
        public void removeWithHashCodesUnsafe(TripleWithHashCodes tripleWithHashCodes) {
            this.removeIf(tripleWithHashCodes.hashes[2],
                    (p) -> {
                        p.removeUnsafe(tripleWithHashCodes.triple, tripleWithHashCodes.hashes[0]);
                        return p.isEmpty();
                    });
        }
    }

    public class TriplesByObject extends FastHashMapBase<TripleSet>  {
        @Override
        protected TripleSet[] createEntryArray(int length) {
            return new TripleSet[length];
        }
        ...
        public void addWithHashCodesUnsafe(TripleWithHashCodes tripleWithHashCodes) {
            this.getOrCreate(tripleWithHashCodes.hashes[3], () -> new TripleSet())
                    .addUnsafe(tripleWithHashCodes.triple, tripleWithHashCodes.hashes[0]);
        }
    ...
    public void add(final Triple triple) {
        ...
        if(this.triplesBySubject.addWithHashCodes(t)) {
            this.triplesByPredicate.addWithHashCodesUnsafe(t);
            this.triplesByObject.addWithHashCodesUnsafe(t);
        }
    }
    ...
    public void remove(final Triple triple) {
        ...
        if(this.triplesBySubject.removeWithHashCodes(t)) {
            this.triplesByPredicate.removeWithHashCodesUnsafe(t);
            this.triplesByObject.removeWithHashCodesUnsafe(t);
        }
    }


    public GraphMem2UsingHashes() {
        super();
    }
    ...
    public void performAdd(final Triple t) {
        ...
        if(this.triplesBySubject.addWithHashCodes(thc)) {
            this.triplesByPredicate.addWithHashCodesUnsafe(thc);
            this.triplesByObject.addWithHashCodesUnsafe(thc);
        }
    }
    ...
    public void performDelete(Triple t) {
        ...
        if(this.triplesBySubject.removeWithHashCodes(thc)) {
            this.triplesByPredicate.removeWithHashCodesUnsafe(thc);
            this.triplesByObject.removeWithHashCodesUnsafe(thc);
        }
    }
            ...
            if(om.isConcrete()) { //SPO:S?0
                if(bySubjectIndex.size() < THRESHOLD_UNTIL_FIND_IS_MORE_EXPENSIVE_THAN_ITERATE) {
                    if(pm.isConcrete()) { // SPO:SPO
                        return bySubjectIndex.contains(triple, thc.hashes[0]);
                    } else { // SPO:S*O
                    ...
                    if(bySubjectIndex.size() <= byObjectIndex.size()) {
                        ...
                        if (pm.isConcrete()) { // SPO:SPO
                            return byObjectIndex.contains(triple, thc.hashes[0]);
                        } else { // SPO:S*O
     ...
     * @return
     */
    private FastHashMapBase<TripleSet> getMapWithFewestKeys() {
    ...
    private static class ListsOfTriplesIterator implements ExtendedIterator<Triple> {
        private static final Iterator<TripleSet> EMPTY_SET_ITERATOR = Collections.emptyIterator();
        private static final Iterator<Triple> EMPTY_TRIPLES_ITERATOR = Collections.emptyIterator();
        private final Graph graph;

        private Iterator<TripleSet> baseIterator;
        private Iterator<Triple> subIterator;
        private Triple current;

        public ListsOfTriplesIterator(Iterator<TripleSet> baseIterator, Graph graph) {
            this.graph = graph;
