 ...
 * has been fixed.
 */
public class GraphMem2Fast2 extends GraphMemBase implements GraphWithPerform {
    ...
    private final HashSetOfTripleSets2 triplesBySubject = new HashSetOfTripleSets2(); //256
    private final HashSetOfTripleSets2 triplesByPredicate = new HashSetOfTripleSets2(); //64
    private final HashSetOfTripleSets2 triplesByObject = new HashSetOfTripleSets2(); //512

    private static int THRESHOLD_FOR_LOW_MEMORY_HASH_SET = 60;//60-350;

    private static abstract class AbstractTriplesListSet extends ListSetBase<Triple> implements TripleSetWithIndexingValue2 {
    ...
    private static class TripleHashSetForSubjects extends FastTripleHashSetWithIndexingValue2 {
        ...
        public TripleHashSetForSubjects(TripleSetWithIndexingValue2 setWithKey) {
            super(setWithKey);
        }
        ...
        @Override
        protected int combineNodeHashes(final int hashCodeOfPredicate, final int hashCodeOfObject) {
            return (31 * hashCodeOfObject) ^ hashCodeOfPredicate;
        }

        @Override
        protected int getHashCodeOfNode1(Triple triple) {
            return triple.getPredicate().getIndexingValue().hashCode();
        }

        @Override
        protected int getHashCodeOfNode2(Triple triple) {
    ...
    private static class TripleHashSetForPredicates extends FastTripleHashSetWithIndexingValue2 {
        ...
        public TripleHashSetForPredicates(TripleSetWithIndexingValue2 setWithKey) {
            super(setWithKey);
        }
        ...
        @Override
        public int combineNodeHashes(final int hashCodeOfSubject, final int hashCodeOfObject) {
            return (127 * hashCodeOfSubject) ^ hashCodeOfObject;
        }

        @Override
        protected int getHashCodeOfNode1(Triple triple) {
            return triple.getSubject().getIndexingValue().hashCode();
        }
    ...
    private static class TripleHashSetForObjects extends FastTripleHashSetWithIndexingValue2 {
        ...
        public TripleHashSetForObjects(TripleSetWithIndexingValue2 setWithKey) {
            super(setWithKey);
        }
        ...
        @Override
        public int combineNodeHashes(final int hashCodeOfSubject, final int hashCodeOfPredicate) {
    ...
    public void performAdd(final Triple t) {
        var hashCodeOfSubject = t.getSubject().getIndexingValue().hashCode();
        var hashCodeOfPredicate = t.getPredicate().getIndexingValue().hashCode();
        var hashCodeOfObject = t.getObject().getIndexingValue().hashCode();
        subject:
        {
            ...
            this.triplesBySubject.compute(
                    ...
                    ts -> {
                        if(ts == null) {
                            ...
                            added[0] = ts.add(t, hashCodeOfPredicate, hashCodeOfObject);
            ...
            this.triplesByPredicate.compute(
                    ...
                    ts -> {
                        if(ts == null) {
                            ...
                            ts.addUnsafe(t, hashCodeOfSubject, hashCodeOfObject);
                        } else if(ts.size() == THRESHOLD_FOR_LOW_MEMORY_HASH_SET) {
            ...
            this.triplesByObject.compute(
                    ...
                    ts -> {
                        if(ts == null) {
                            ...
                            ts.addUnsafe(t, hashCodeOfSubject, hashCodeOfPredicate);
                        } else if(ts.size() == THRESHOLD_FOR_LOW_MEMORY_HASH_SET) {
    ...
    public void performDelete(Triple t) {
                    ...
                    ts -> {
                        if(ts == null) {
                            return null;
                                ...
                                ? ts.remove(t, hashCodeOfPredicate, hashCodeOfObject)
                                : ts.remove(t)) {
                            removed[0] = true;
                            if(ts.isEmpty()) {
            ...
            this.triplesByPredicate.compute(
                    ...
                    ts -> {
                        if(ts.areOperationsWithHashCodesSupported()) {
                            ts.removeUnsafe(t, hashCodeOfSubject, hashCodeOfObject);
                        } else {
                            ts.removeUnsafe(t);
                        }
        ...
        object:
        {
            this.triplesByObject.compute(
                    ...
                    ts -> {
                        if(ts.areOperationsWithHashCodesSupported()) {
                            ts.removeUnsafe(t, hashCodeOfSubject, hashCodeOfPredicate);
                        } else {
                            ts.removeUnsafe(t);
                        }
                        return ts.isEmpty() ? null : ts;
     ...
     * @return
     */
    private HashSetOfTripleSets2 getMapWithFewestKeys() {
    ...
    public ExtendedIterator<Triple> graphBaseFind(Triple triplePattern) {
        ...
        if (sm.isConcrete()) { // SPO:S??
            ...
            if(om.isConcrete()) { //SPO:S?0
                if(bySubjectIndex.size() < THRESHOLD_UNTIL_FIND_IS_MORE_EXPENSIVE_THAN_ITERATE) {
                    if(pm.isConcrete()) { // SPO:SPO
                        return new IteratorFiltering(
                                bySubjectIndex.areOperationsWithHashCodesSupported()
                                    ? bySubjectIndex.iterator(
                        ...
                        return new IteratorFiltering(
                                bySubjectIndex.areOperationsWithHashCodesSupported()
                                        ? bySubjectIndex.iteratorByNode2(
                                            om.getIndexingValue().hashCode())
                                        : bySubjectIndex.iterator(),
                                t -> om.sameValueAs(t.getObject()),
                                this);
                    ...
                    if(bySubjectIndex.size() <= byObjectIndex.size()) {
                        ...
                        if (pm.isConcrete()) { // SPO:SPO
                            return new IteratorFiltering(
                                    byObjectIndex.areOperationsWithHashCodesSupported()
                                        ? byObjectIndex.iterator(
                ...
                return new IteratorFiltering(
                        bySubjectIndex.areOperationsWithHashCodesSupported()
                                ? bySubjectIndex.iteratorByNode1(
                                    pm.getIndexingValue().hashCode())
                                : bySubjectIndex.iterator(),
                        t -> pm.equals(t.getPredicate()),
                        this);
            ...
            if(pm.isConcrete()) { // SPO:*PO
                if(byObjectIndex.size() < THRESHOLD_UNTIL_FIND_IS_MORE_EXPENSIVE_THAN_ITERATE) {
                    ...
                    if(byObjectIndex.size() <= byPredicateIndex.size()) {
                        ...
                        return new IteratorFiltering(
                                byPredicateIndex.areOperationsWithHashCodesSupported()
                                        ? byPredicateIndex.iteratorByNode2(
                                            om.getIndexingValue().hashCode())
                                        : byPredicateIndex.iterator(),
                                t -> om.sameValueAs(t.getObject()),
                                this);
                    }
                }
    ...
    private static class ListsOfTriplesIterator implements ExtendedIterator<Triple> {
        private static final Iterator<TripleSetWithIndexingValue2> EMPTY_SET_ITERATOR = Collections.emptyIterator();
        private static final Iterator<Triple> EMPTY_TRIPLES_ITERATOR = Collections.emptyIterator();
        private final Graph graph;

        private Iterator<TripleSetWithIndexingValue2> baseIterator;
        private Iterator<Triple> subIterator;
        private Triple current;

        public ListsOfTriplesIterator(Iterator<TripleSetWithIndexingValue2> baseIterator, Graph graph) {
            this.graph = graph;
