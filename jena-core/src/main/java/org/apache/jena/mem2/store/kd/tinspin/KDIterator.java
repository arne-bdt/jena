...
import static org.apache.jena.mem2.store.kd.tinspin.Index.PointIterator;
...
public class KDIterator<T> implements PointIterator<T> {
	...
	private static class IteratorPos<T> {
		...
		void set(Node<T> node, int[] min, int[] max, int depth, int dims) {
			this.node = node;
			this.depth = depth;
			int[] key = node.point();
			int pos = depth % dims;
	...
	private class IteratorStack {
		...
		void prepareAndPush(Node<T> node, int[] min, int[] max, int depth, int dims) {
			if (size == stack.size()) {
	...
	KDIterator(KDTree<T> tree, int[] min, int[] max) {
	 ...
	 */
	@Override
	public PointIterator<T> reset(int[] min, int[] max) {
		stack.clear();
		this.min = min;
		this.max = max;
		next = null;
