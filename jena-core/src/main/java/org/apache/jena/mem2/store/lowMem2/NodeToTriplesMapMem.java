...
public class NodeToTriplesMapMem implements NodeToTriplesMap {
    ...
    private static final int MAX_ARRAY_BUNCH_SIZE = 64;
    private final JenaMap<Node, TripleBunch> bunchMap = new LowMemHashedBunchMap();
    ...
    public boolean tryAdd(Triple t) {
        ...
        if ((!s.isHashed()) && s.size() == MAX_ARRAY_BUNCH_SIZE) {
    ...
    public void addUnchecked(Triple t) {
        ...
        if (s == null) {
            ...
            s.addUnchecked(t);
            bunchMap.put(node, s);
        } else if ((!s.isHashed()) && s.size() == MAX_ARRAY_BUNCH_SIZE) {
    ...
    private class ArrayBunch extends FastArrayBunch implements TripleBunch {

        @Override
        public Node getIndexingNode() {
            return indexField.getField(elements[0]);
        }
    }
}
