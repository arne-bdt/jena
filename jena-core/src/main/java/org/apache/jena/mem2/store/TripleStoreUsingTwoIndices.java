...
import org.apache.jena.mem2.map.TriplesMapWithTwoIndices;
import org.apache.jena.mem2.map.TriplesMapWithTwoIndicesUsingFastHashSets;
import org.apache.jena.mem2.pattern.PatternClassifier;

import java.util.Iterator;
import java.util.stream.Stream;

public class TripleStoreUsingTwoIndices implements TripleStore {

    final TriplesMapWithTwoIndices spo = new TriplesMapWithTwoIndicesUsingFastHashSets();
    final TriplesMapWithTwoIndices pos = new TriplesMapWithTwoIndicesUsingFastHashSets();
    final TriplesMapWithTwoIndices osp = new TriplesMapWithTwoIndicesUsingFastHashSets();

    @Override
    public void add(Triple triple) {
        var hashes = getHashCodes(triple);
        if(this.spo.add(hashes[1], hashes[2], hashes[0], triple)) { // if it was not already present, add it to the other two maps without checking
            this.pos.addWithoutChecking(hashes[2], hashes[3], hashes[0], triple);
            this.osp.addWithoutChecking(hashes[3], hashes[1], hashes[0], triple);
        }
    }
    ...
    public void remove(Triple triple) {
        var hashes = getHashCodes(triple);
        if(this.spo.remove(hashes[1], hashes[2], hashes[0], triple)) { // if it exists in the fist map, it exists in the other two
            this.pos.removeWithoutChecking(hashes[2], hashes[3], hashes[0], triple);
            this.osp.removeWithoutChecking(hashes[3], hashes[1], hashes[0], triple);
        }
    }
        ...
        switch (PatternClassifier.classify(sm, pm, om)) {
            case SPO:
                return this.spo.stream(
                        ...
                        .filter(triple ->
                                om.sameValueAs(triple.getObject())
     ...
     * @return
     */
    private TriplesMapWithTwoIndices getMapWithFewestTopLevelIndices() {
