...
import static org.apache.jena.mem2.store.kd.tinspin.Index.*;
...
public class KDIteratorKnn<T> implements PointIteratorKnn<T> {
    ...
    KDIteratorKnn(Node<T> root, int minResults, int[] center, PointDistance distFn, PointFilterKnn<T> filterFn) {
    ...
    @Override
    public PointIteratorKnn<T> reset(int[] center, int minResults) {
        this.center = center;
    ...
    void createEntryHi(NodeDist<T> entry) {
        ...
        int splitX = node.point()[splitDim];
    ...
    private static class NodeDist<T> {
        ...
        int[] closest;
        NodeDist(double closestDist, Node<T> node, int[] closest) {
