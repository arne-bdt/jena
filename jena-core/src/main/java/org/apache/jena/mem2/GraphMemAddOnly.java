...
/**
 * GraphMemAddOnly uses very very little memory and is very fast for adding triples.
 * With increasing size, deleting triples becomes slower and slower.
 *
 ...
 * - GraphMemAddOnly optimizes find operations by
 ...
 * - GraphMemAddOnly has the Graph#stream operations implemented as real java streams considering the same
 *   optimizations as the find operations and not wrapping iterators to streams.
 * - GraphMemAddOnly optimizes memory usage by using Node.getIndexingValue().hashCode() as hash keys instead
 ...
 * has been fixed.
 */
public class GraphMemAddOnly extends GraphMemBase implements GraphWithPerform {

    private static final int INITIAL_SIZE_FOR_ARRAY_LISTS = 2;

    private final Map<Integer, ArrayList<Triple>> triplesBySubject = new HashMap<>();
    private final Map<Integer, ArrayList<Triple>> triplesByPredicate = new HashMap<>();
    private final Map<Integer, ArrayList<Triple>> triplesByObject = new HashMap<>();

    public GraphMemAddOnly() {
        super();
    }
    ...
    public void performAdd(Triple t) {
        ...
        var sList = this.triplesBySubject.get(sIdx);
        if(sList != null) {
            if(sList.contains(t)) {
                return; /*triple already exists*/
            }
        } else {
            sList = new ArrayList<>(INITIAL_SIZE_FOR_ARRAY_LISTS);
            this.triplesBySubject.put(sIdx, sList);
        }
        sList.add(t);
        {
            var pIdx = t.getPredicate().getIndexingValue().hashCode();
            var pList = this.triplesByPredicate
                    .computeIfAbsent(pIdx,
                            key -> new ArrayList<>(INITIAL_SIZE_FOR_ARRAY_LISTS));
            pList.add(t);
        }
        {
            var oIdx = t.getObject().getIndexingValue().hashCode();
            var oList = this.triplesByObject
                    .computeIfAbsent(oIdx,
                            key -> new ArrayList<>(INITIAL_SIZE_FOR_ARRAY_LISTS));
            oList.add(t);
        }
    }
    ...
    public void performDelete(Triple t) {
        ...
        var sList = this.triplesBySubject.get(sIdx);
        if(sList == null) {
            return; /*triple does not exist*/
        }
        if(sList.remove(t)) {
            if(sList.isEmpty()) {
                this.triplesBySubject.remove(sIdx);
            }
            {
                var pIdx = t.getPredicate().getIndexingValue().hashCode();
                var pList = this.triplesByPredicate.get(pIdx);
                pList.remove(t);
                if (pList.isEmpty()) {
                    this.triplesByPredicate.remove(pIdx);
                }
            }
            {
                var oIdx = t.getObject().getIndexingValue().hashCode();
                var oList = this.triplesByObject.get(oIdx);
                oList.remove(t);
                if (oList.isEmpty()) {
                    this.triplesByObject.remove(oIdx);
                }
            }
        }
    }
    ...
    private static class ListsOfTriplesIterator implements Iterator<Triple> {
        ...
        private final Iterator<ArrayList<Triple>> baseIterator;
        private Iterator<Triple> subIterator;
        private boolean hasSubIterator = false;

        public ListsOfTriplesIterator(Iterator<ArrayList<Triple>> baseIterator) {
    ...
    private static class IteratorWrapperWithRemove implements ExtendedIterator<Triple> {
        ...
        private final GraphMemAddOnly graphMem;
        ...
        public IteratorWrapperWithRemove(Iterator<Triple> iteratorWithNoRemove, GraphMemAddOnly graphMem) {
