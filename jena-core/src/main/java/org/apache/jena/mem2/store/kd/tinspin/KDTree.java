...
public class KDTree<T> implements PointMap<T>, PointMultimap<T> {
	 ...
	 */
	@Override
	public KDIterator<T> queryExactPoint(int[] point) {
	 ...
	 */
	@Override
	public T queryExact(int[] key) {
		Node<T> e = findNodeExact(key, new RemoveResult<>(), entry -> true);
		return e == null ? null : e.value();
	}

	private Node<T> findNodeExact(int[] key, RemoveResult<T> resultDepth, Predicate<PointEntry<T>> filter) {
		if (root == null) {
			return null;
		}
	...
	private Node<T> findNodeExactFast(int[] key, Node<T> parent, RemoveResult<T> resultDepth, Predicate<PointEntry<T>> filter) {
		Node<T> n = root;
		do {
			int[] nodeKey = n.point();
			int nodeX = nodeKey[n.getDim()];
			int keyX = key[n.getDim()];
	...
	private Node<T> findNodeExactSlow(int[] key, Node<T> n, Node<T> parent, RemoveResult<T> resultDepth, Predicate<PointEntry<T>> filter) {
		do {
			int[] nodeKey = n.point();
			int nodeX = nodeKey[n.getDim()];
			int keyX = key[n.getDim()];
	...
	@Override
	public boolean removeIf(int[] key, Predicate<PointEntry<T>> pred) {
		if (root == null) {
			return false;
		}
...
//			}
	 ...
	 */
	@Override
	public T update(int[] oldKey, int[] newKey) {
		if (root == null) {
			return null;
		}
	 ...
	 */
	@Override
	public boolean update(int[] oldKey, int[] newKey, T value) {
		if (root == null) {
			return false;
		}
	 ...
	 */
	@Override
	public KDIterator<T> query(int[] min, int[] max) {
		return new KDIterator<>(this, min, max);
	}

	static boolean isEnclosed(int[] point, int[] min, int[] max) {
	...
	private List<PointEntryKnn<T>> knnQuery(int[] center, int k, PointDistance distFn) {
		if (root == null) {
	...
	private double rangeSearchKNN(Node<T> node, int[] center,
    ...
    private double addCandidate(Node<T> node, int[] center,
                                ArrayList<PointEntryKnn<T>> candidates, int k, double maxRange, PointDistance distFn) {
    	nDistKNN++;
    	//add ?
    ...
    private static class KDQueryIteratorKnn<T> implements PointIteratorKnn<T> {
		...
		public KDQueryIteratorKnn(KDTree<T> tree, int[] center, int k, PointDistance distFn) {
			this.tree = tree;
			this.distFn = distFn;
			reset(center, k);
		}
		...
		@Override
		public KDQueryIteratorKnn<T> reset(int[] center, int k) {
	...
	public PointIterator<T> iterator() {
		if (root == null) {
			return query(new int[dims], new int[dims]);
		}
		//return query(root.);
		//TODO
	...
	@Override
	public PointEntryKnn<T> query1nn(int[] center) {
		return queryKnn(center, 1).next();
	}

	@Override
	public PointIteratorKnn<T> queryKnn(int[] center, int k) {
	...
	@Override
	public PointIteratorKnn<T> queryKnn(int[] center, int k, PointDistance distFn) {
