 ...
 */
public class KeyValueLowMemoryHashSet<K, V extends KeyValueLowMemoryHashSet.KeyedValue<K>> implements Set<V> {

    public interface KeyedValue<K> {
        K getKey();
        int getHashCodeOfKey();
    }
    ...
    public KeyValueLowMemoryHashSet() {
        this.entries = new Object[MINIMUM_SIZE];
    }

    public KeyValueLowMemoryHashSet(int initialCapacity) {
        this.entries = new Object[Integer.highestOneBit(((int)(initialCapacity/loadFactor)+1)) << 1];
    }

    public KeyValueLowMemoryHashSet(Set<? extends V> set) {
        this(set.size(), set);
    }

    public KeyValueLowMemoryHashSet(int initialCapacity, Set<? extends V> set) {
        this.entries = new Object[Integer.highestOneBit(((int)(Math.max(set.size(), initialCapacity)/loadFactor)+1)) << 1];
        int index;
        for (V e : set) {
            if((index = findIndex(e.getKey(), e.getHashCodeOfKey())) < 0) {
                entries[~index] = e;
                size++;
            }
        }
    ...
    private void grow(final int minCapacity) {
        ...
        for(int i=0; i<oldEntries.length; i++) {
            if(null != oldEntries[i]) {
                this.entries[findEmptySlotWithoutEqualityCheck(((V)oldEntries[i]).getHashCodeOfKey())] = oldEntries[i];
            }
        }
    }
    ...
    public boolean contains(Object o) {
        ...
        final var hashCode = e.getHashCodeOfKey();
        var index = calcStartIndexByHashCode(hashCode);
        if(null == entries[index]) {
            return false;
        }
        var entry = (V)entries[index];
        if(hashCode == entry.getHashCodeOfKey() && key.equals(entry.getKey())) {
            return true;
        } else if(--index < 0){
    ...
    public boolean add(V value) {
        grow();
        var index = findIndex(value.getKey(), value.getHashCodeOfKey());
        if(index < 0) {
            entries[~index] = value;
            size++;
            return true;
    ...
    public void addUnsafe(V value) {
        grow();
        entries[findEmptySlotWithoutEqualityCheck(value.getHashCodeOfKey())] = value;
        size++;
    }
    ...
    private int findIndex(final K key, final int hashCode) {
        ...
        while(true) {
            if(null == entries[index]) {
                return ~index;
            } else if(hashCode == ((V)entries[index]).getHashCodeOfKey() && key.equals(((V)entries[index]).getKey())) {
                return index;
            } else if(--index < 0){
                index += entries.length;
            }
    ...
    public void removeUnsafe(V e) {
        var index = findIndex(e.getKey(), e.getHashCodeOfKey());
        entries[index] = null;
        size--;
        rearrangeNeighbours(index);
    }
    ...
    private ObjectsWithStartIndexIndexAndDistance[] getNeighbours(final int index) {
        ...
        while(true) {
            ...
            if(null == entries[i]) {
                ...
                neighbour = new ObjectsWithStartIndexIndexAndDistance(entries.length, calcStartIndexByHashCode(((V)entries[i]).getHashCodeOfKey()), i);
                if(neighbour.distance > 0) {
