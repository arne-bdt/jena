...
public abstract class HashCommon<Key>
    ...
    */
    protected final int initialIndexFor( int hashCodeOfKey )
        { return (improveHashCode( hashCodeOfKey ) & 0x7fffffff) % keys.length; }
    ...
    protected Key removeFrom( int here )
        ...
        while (true)
            ...
            while (true)
                ...
                int r = initialIndexFor( key.hashCode() );
    ...
    void showkeys()
        ...
        if (false)
            ...
            for (int i = 0; i < keys.length; i += 1)
                if (keys[i] != null) System.err.print( " " + initialIndexFor( keys[i].hashCode() ) + "@" + i + "::" + keys[i] );
            System.err.println();
            }
        }
